...
export async function handleExpenseList(request: globalThis.Request) { const auth = await authenticateRequest(request); return listExpenses(supabaseForAccessToken(auth.accessToken), auth.user); }
