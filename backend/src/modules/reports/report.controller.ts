...
export async function handleReportSummary(request: globalThis.Request, body: unknown) { const auth = await authenticateRequest(request); return reportSummary(supabaseForAccessToken(auth.accessToken), auth.user, body); }
