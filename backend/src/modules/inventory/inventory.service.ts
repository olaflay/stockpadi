...
export async function upsertProduct(db: SupabaseClient, actor: User, input: unknown) {
  ...
  if (context.accountType !== "BUSINESS_OWNER") throw new HttpError(403, "FORBIDDEN", "Only a business owner can change products");
...
export async function adjustStock(db: SupabaseClient, actor: User, input: unknown) {
  ...
  if (context.accountType === "WORKER" && !context.branchIds.includes(payload.branchId)) throw new HttpError(403, "FORBIDDEN", "Branch is outside this account's assigned branches");
