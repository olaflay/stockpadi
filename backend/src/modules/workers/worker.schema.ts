...
export type WorkerAction = "create" | "reset_password" | "deactivate" | "reactivate";
...
export function parseWorkerRequest(input: unknown): WorkerRequest {
  ...
  if (body.action !== "create" && body.action !== "reset_password" && body.action !== "deactivate" && body.action !== "reactivate") throw new HttpError(400, "INVALID_BODY", "Invalid worker action");
