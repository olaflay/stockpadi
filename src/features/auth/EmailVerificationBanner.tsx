...
async function callVerifyEmail(code: string): Promise<{ ok: boolean; message?: string }> {
  ...
  if (!res.ok) return { ok: false, message: json?.error?.message ?? "That did not work. Try again." };
  return { ok: true };
}
...
async function callSendVerification(): Promise<{ ok: boolean; message?: string }> {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-verification`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    }
  ...
  const json = await res.json();
  if (!res.ok) return { ok: false, message: json?.error?.message ?? "Could not send the email. Try again shortly." };
  return { ok: true };
}
...
export function EmailVerificationBanner({ userId }: EmailVerificationBannerProps) {
  ...
  async function handleVerify() {
    ...
    setBusy(true);
    try {
      const result = await callVerifyEmail(code);
      if (!result.ok) {
