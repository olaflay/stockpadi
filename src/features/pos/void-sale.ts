...
export async function voidSale(params: { saleId: string; branchId: string; reason?: string }): Promise<void> {
  ...
  const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/void-sale`;
  ...
  const originalMovements = await db.stockMovements
    .filter((m) => m.sourceReferenceId === params.saleId && m.source === "sale")
    .toArray();

  const originalCreditMovements = await db.customerCreditMovements
  ...
  await db.transaction("rw", db.sales, db.stockMovements, db.customerCreditMovements, async () => {
    ...
    if (originalMovements.length > 0) {
      ...
      await db.stockMovements.bulkAdd(reversals);
    }
    if (originalCreditMovements.length > 0) {
      ...
      await db.customerCreditMovements.bulkAdd(creditReversals);
    }
  });
}
