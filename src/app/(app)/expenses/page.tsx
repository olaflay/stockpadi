...
const CAN_VIEW_EXPENSES = ["owner", "manager", "accountant", "admin"] as const;
...
const CAN_DELETE_EXPENSES = ["owner", "accountant", "admin"] as const;
...
export default function ExpensesPage() {
  ...
  const result = useLiveQuery(async () => {
    try {
      const [expenses, branches, users] = await Promise.all([
        db.expenses.orderBy("createdAtLocal").reverse().toArray(),
        db.branches.toArray(),
  ...
  if (!hasRole(user, [...CAN_VIEW_EXPENSES])) {
    return (
      <div>
        <ScreenHeader title="Expenses" onBack={() => router.push("/reports")} />
        <PermissionDenied requiredRoles={[...CAN_VIEW_EXPENSES]} />
      </div>
    );
  }
  ...
  const canDelete = hasRole(user, [...CAN_DELETE_EXPENSES]);
