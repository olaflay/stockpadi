...
const CAN_VIEW_CUSTOMER_DETAIL = ["owner", "manager", "cashier", "accountant", "admin"] as const;
...
export default function CustomerDetailPage({ params }: PageProps) {
  ...
  const result = useLiveQuery(async () => {
    try {
      const [customer, movements, balance] = await Promise.all([
        db.customers.get(id),
        db.customerCreditMovements.where("customerId").equals(id).toArray(),
  ...
  if (!hasRole(user, [...CAN_VIEW_CUSTOMER_DETAIL])) {
    return (
      <div>
        <ScreenHeader title="Customer" onBack={() => router.push("/customers")} />
        <PermissionDenied requiredRoles={[...CAN_VIEW_CUSTOMER_DETAIL]} />
      </div>
    );
  }
