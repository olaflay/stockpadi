...
export default function DashboardPage() {
  ...
  const canSeeMoney = roleHasPermission(user.role, "view_sales");
  const viewerId = user.role === "cashier" ? user.id : null;
  ...
  const localUser = useLiveQuery(() => db.localUsers.get(user.id), [user.id]);
  const showVerificationBanner =
    user.role === "owner" && localUser !== undefined && !localUser?.emailVerified;
    ...
    return (
      <div className="flex flex-col h-screen">
        ...
        <EmptyState
          ...
          action={{
            label: "Add a product",
            onClick: () => router.push(hasRole(user, [...CAN_EDIT_PRODUCTS]) ? "/welcome" : "/products"),
          }}
          fullScreen
        />
      </div>
  ...
  return (
    <div>
      ...
      {(hasRole(user, [...CAN_CLOSE_DAY]) || metrics.unsyncedCount > 0) && (
        <div className="mt-4 flex flex-col gap-3">
          {hasRole(user, [...CAN_CLOSE_DAY]) && (
            <RippleButton
              type="button"
