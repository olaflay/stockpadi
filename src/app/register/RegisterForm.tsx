...
export default function RegisterForm({ csrfToken }: { csrfToken: string }) {
  ...
  const isPinValid = pin === confirmPin && /^\d{4}$/.test(pin);
  // Only flag once both fields are fully entered — not while still typing.
  const pinMismatch = pin.length === 4 && confirmPin.length === 4 && pin !== confirmPin;
  ...
  async function handleSubmit() {
    ...
    setBusy(true);
    try {
      // 1. Call Server Action to handle idempotent creation with pool
      const actionResult = await createAccountAction({
        email,
        password,
        fullName,
        businessName,
      ...
      if (!actionResult.success) {
        setError(actionResult.error || "Failed to create account. Please try again.");
        return;
      }
      ...
      // 5. Persist PIN hash server-side
      try {
    ...
    <div className="flex h-screen w-full flex-col max-w-md mx-auto overflow-hidden">
        ...
        <div className="flex flex-col gap-4 mt-5">
          ...
          <div className="flex flex-col gap-3 rounded-[var(--radius-card)] border border-border bg-surface-container/40 p-4">
            ...
            <p className="text-[length:var(--font-size-caption)] text-on-surface-muted">
              Used to unlock the register daily.
            </p>
            ...
            <div className="flex gap-4">
              ...
              <label className="flex flex-col gap-1.5 flex-1">
                ...
                <TextInput
                  ...
                  errorId="register-pin-mismatch"
                  className="text-center tracking-widest text-lg"
                />
              </label>
            </div>
            {pinMismatch && (
              <span id="register-pin-mismatch" role="alert" className="text-[length:var(--font-size-caption)] text-danger">
                PINs don&apos;t match.
              </span>
            )}
          </div>
      ...
      <div className="flex flex-col gap-3 border-t border-border px-6 py-4 shrink-0">
        <RippleButton
          ...
          disabled={busy || !formComplete || !isPinValid}
