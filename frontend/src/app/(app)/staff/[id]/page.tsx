...
export default function StaffDetailPage({ params }: PageProps) {
  ...
  async function resetWorkerPassword() {
    ...
    setBusy(true);
    try {
      const result = await callManageStaff({ action: "reset_password", userId: member.id });
      setNewPassword(result.password ?? null);
    } catch (err) {
      ...
      <div className="flex flex-col gap-2 rounded-[var(--radius-card)] border border-border bg-surface-container/40 p-4">
        ...
        {newPassword && (
          <div className="flex items-center justify-between gap-2 rounded-[var(--radius-control)] border border-border bg-surface-container p-3">
            ...
            <RippleButton type="button" onClick={() => navigator.clipboard?.writeText(newPassword)} className="min-h-[var(--touch-target-min)] rounded-[var(--radius-control)] border border-brand-accent px-4 font-semibold text-brand-accent">
              Copy
            </RippleButton>
          </div>
        )}
