  ...
  if (variant === "pill-icons") {
    return (
      <div
        ...
        className={`inline-flex items-center gap-0.5 rounded-full bg-surface-container-high p-0.5 ${className}`}
      >
          ...
          return (
            <button
              ...
              className={`flex h-6.5 w-6.5 items-center justify-center rounded-full transition-all ${
                selected
                  ? "bg-surface text-brand-accent scale-105"
  ...
  if (variant === "capsule-pill") {
    return (
      <div
        role="radiogroup"
        aria-label="Appearance"
        className={`inline-flex items-center rounded-full bg-surface-container-high p-1 ${className}`}
      >
        {OPTIONS.map(({ value, label, icon: Icon }) => {
          const selected = theme === value;
          return (
            <button
              ...
              className={`relative flex items-center justify-center rounded-full transition-all duration-200 ${
                selected
                  ? "bg-surface text-on-surface px-2.5 sm:px-3.5 py-1 sm:py-1.5 gap-1.5 font-semibold text-xs sm:text-sm"
        ...
        return (
          <button
            ...
            className={`flex min-h-[var(--touch-target-min)] flex-1 flex-col items-center justify-center gap-1 rounded-[var(--radius-inline)] px-2 py-2 text-[length:var(--font-size-caption)] transition-colors ${
              selected
                ? "bg-surface text-brand-accent font-[var(--font-weight-emphasis)]"
