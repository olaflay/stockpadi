...
1. ~~**Import v1.1 (semantics):** blank-SKU auto-generation (mirror `generateFallbackSku`, stable tail, dedupe incl. generated), core-cell blocking per all-or-nothing, downloadable error-report CSV, format/limits hint on the drop zone, success toast with "now syncing in background", button copy fix ("CSV template" not "Excel / CSV" unless we ship Excel).~~ **Done.** `lowStockThreshold` left optional so a blank cell imports as `null` (not a surprise 5).
2. ~~**Import v1.2 (Excel):** add `exceljs` (trusted-tool research first), normalize first sheet → `ParsedCsvRow` pipeline, dual-template download (.csv + .xlsx), `accept=".csv,.xlsx"`.~~ **Done.** `exceljs@4.4.0` (MIT, browser field → webpack pulls the self-contained UMD bundle as a **910KB lazy chunk**, fetched only when a template is downloaded or a file uploaded — never in the offline main shell). New module `features/inventory/product-import.ts` replaces `csv-import.ts`: fixed `IMPORT_HEADERS`, `buildSampleExcel()` (frozen + bold header row, two example rows, xlsx blob), `parseProductFile()` with **content auto-detect** (.xlsx by extension or `PK\x03\x04` magic bytes → `exceljs`; anything else → Papa text path), and the same all-or-nothing `validateRawRows` core for both formats. Page updated: "Download template file (Excel .xlsx)", `accept=".xlsx,.txt,.csv"`, xlsx-safe error toast. 5 new Excel tests (valid workbook, reordered headers, typed numeric cells, blank core cell, template blob magic bytes) → 178 total pass; typecheck + `next build --webpack` clean. Brands/categories/alt units still out of scope (brands have no client model — nothing to map to).
...
- Code: `products/import/page.tsx`, `features/inventory/product-import.ts`, `features/inventory/import-products.ts`, `features/inventory/use-new-product-form.ts` (SKU fallback), `shells/ExperienceShell.tsx`, `components/ui/SideDrawer.tsx`, `config/limits.ts`.