...
Schema, RLS policies, and the `sync_apply_*` functions live in `supabase/migrations/`. Apply them against a local or linked Supabase Cloud project with the [Supabase CLI](https://supabase.com/docs/guides/cli):

```bash
npx supabase start        # local instance, requires Docker
npx supabase db reset      # local
npx supabase link --project-ref <ref>
npx supabase db push       # apply migrations to the linked Supabase Cloud project
```
...
1. `npx supabase db reset` (or `npx supabase db push` to the target project) and confirm it completes with no errors.
